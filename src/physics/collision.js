 ...
 * @ignore
 */
let LEFT_VORNOI_REGION = -1;

/**
 * Constants for Vornoi regions
 * @ignore
 */
let MIDDLE_VORNOI_REGION = 0;

/**
 * Constants for Vornoi regions
 * @ignore
 */
let RIGHT_VORNOI_REGION = 1;
 ...
 */
 let flattenPointsOn = function(points, normal, result) {
 ...
 */
let isSeparatingAxis = function(aPos, bPos, aPoints, bPoints, axis, response) {
 ...
 */
let vornoiRegion = function(line, point) {
...
/**
 * set of API for collision detection (and projection-based collision response) of 2D shapes.<br>
...
export function init () {
    ...
    bounds = me.game.viewport.clone();
    // initializa the quadtree
    quadTree = new me.QuadTree(bounds, maxChildren, maxDepth);
 ...
 */
export let response = new ResponseObject();
 ...
 * },
 */
export function check(objA, responseObject) {
    var collision = 0;
    var response = responseObject || response;

    // retreive a list of potential colliding objects
    var candidates = quadTree.retrieve(objA);
...
export function rayCast(line, resultArray) {
    ...
    var candidates = quadTree.retrieve(line.getBounds());
