 ...
 * @ignore
 */
let checkLoadStatus = function(onload) {
 ...
 * @ignore
 */
let preloadImage = function(img, onload, onerror) {
 ...
 * @ignore
 */
let preloadTMX = function(tmxData, onload, onerror) {
        ...
        if (xmlhttp.readyState === 4) {
            ...
            if ((xmlhttp.status === 200) || ((xmlhttp.status === 0) && xmlhttp.responseText)) {
                ...
                switch (format) {
                    ...
                    case "tsx":
                        ...
                        if (me.device.ua.match(/msie/i) || !xmlhttp.responseXML) {
                            if (window.DOMParser) {
                                ...
                                throw new Error("me.audio: XML file format loading not supported, use the JSON file format instead");
                            }
                        }
                        else {
                    ...
                    default:
                        throw new Error("me.audio: TMX file format " + format + "not supported !");
                }
 ...
 * @ignore
 */
let preloadJSON = function(data, onload, onerror) {
    var xmlhttp = new XMLHttpRequest();
 ...
 * @ignore
 */
let preloadBinary = function(data, onload, onerror) {
 ...
 * @ignore
 */
export function onResourceLoaded (res) {
 ...
 * @ignore
 */
export function onLoadingError (res) {
    throw new Error("me.audio: Failed loading resource " + res.src);
};
 ...
 * @ignore
 */
export function setNocache (enable) {
 ...
 * me.loader.preload(game.resources, this.loaded.bind(this));
 */
export function preload (res, onloadcb, switchToLoadState) {
    // parse the resources
...
export function load (res, onload, onerror) {
    ...
    switch (res.type) {
        ...
        default:
            throw new Error("me.audio: load : unknown or invalid resource type : " + res.type);
    }
};
...
export function unload (res) {
    switch (res.type) {
        ...
        default:
            throw new Error("me.audio: unload : unknown or invalid resource type : " + res.type);
    }
};
