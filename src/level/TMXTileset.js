...
let TMX_CLEAR_BIT_MASK = ~(0x80000000 | 0x40000000 | 0x20000000);
    ...
    constructor(tileset) {
        ...
        if (typeof(tileset.source) !== "undefined") {
            ...
            if (ext === "tsx" || ext === "json") {
                ...
                if (!tileset) {
                    throw new me.Error("me.TMXTileset: " + src + " external TSX/JSON tileset not found");
                }
            }
        }
        ...
        for (i in tiles) {
            if (tiles.hasOwnProperty(i)) {
                ...
                if ("image" in tiles[i]) {
                    ...
                    if (!image) {
                        throw new me.Error("me.TMXTileset: '" + tiles[i].image + "' file for tile '" + (+i + this.firstgid) + "' not found!");
                    }
        ...
        if (this.isCollection === false) {
            ...
            if (!this.image) {
                throw new me.Error("me.TMXTileset: '" + tileset.image + "' file for tileset '" + this.name + "' not found!");
            }
...
export class TMXTilesetGroup {
    ...
    getTilesetByGid(gid) {
        ...
        if (invalidRange !== -1) {
            ...
            throw new me.Error("me.TMXTileset: no matching tileset found for gid " + gid);
        }
    }
};
