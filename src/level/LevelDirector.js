...
let safeLoadLevel = function(levelId, options, restart) {
    ...
    if (levels[getCurrentLevelId()]) {
        levels[getCurrentLevelId()].destroy();
    }
 ...
 * @function
 */
let loadTMXLevel = function (levelId, container, flatten, setViewportBounds) {
 ...
 * @ignore
 */
export function addTMXLevel(levelId, callback) {
 ...
 */
export function loadLevel(levelId, options) {
 ...
 * @param {boolean} [options.flatten=me.game.mergeGroup] if true, flatten all objects into the given container
 */
export function reloadLevel(options) {
    // reset the level to initial state
    //levels[currentLevel].reset();
    return loadLevel(getCurrentLevelId(), options);
}
 ...
 * @param {boolean} [options.flatten=me.game.mergeGroup] if true, flatten all objects into the given container
 */
export function nextLevel(options) {
    //go to the next level
    if (currentLevelIdx + 1 < levelIdx.length) {
        return loadLevel(levelIdx[currentLevelIdx + 1], options);
    }
    else {
        return false;
    }
 ...
 * @param {boolean} [options.flatten=me.game.mergeGroup] if true, flatten all objects into the given container
 */
export function previousLevel(options) {
