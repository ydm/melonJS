...
export default class BitmapFontData {
    ...
    _getValueFromPair(string, pattern) {
        ...
        if (!value) {
            throw "me.BitmapFontData: Could not find pattern " + pattern + " in string: " + string;
        }
    ...
    parse(fontData) {
        if (!fontData) {
            throw "me.BitmapFontData: File containing font data was empty, cannot load the bitmap font.";
        }
        var lines = fontData.split(/\r\n|\n/);
        var padding = fontData.match(/padding\=\d+,\d+,\d+,\d+/g);
        if (!padding) {
            throw "me.BitmapFontData: Padding not found in first line";
        }
